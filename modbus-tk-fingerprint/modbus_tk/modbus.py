...
class Master(object):
    ...
    def execute(
        ...
        if function_code == defines.READ_COILS or function_code == defines.READ_DISCRETE_INPUTS:
            ...
            pdu = struct.pack(">BB", function_code, count_seq * 7) + b''.join(
                map(lambda zip_param: struct.pack(">BHHH", *zip_param),
                    zip(count_seq * (6,), number_file, starting_address, quantity_of_x)))
            if not data_format:
...
class Slave(object):
    ...
    def _read_write_multiple_registers(self, request_pdu):
        ...
        (starting_read_address, quantity_of_x_to_read, starting_write_address, quantity_of_x_to_write,
         byte_count_to_write) = struct.unpack(">HHHHB", request_pdu[1:10])
        ...
        # look for the block corresponding to the request
        block, offset = self._get_block_and_offset(defines.HOLDING_REGISTERS, starting_read_address,
                                                   quantity_of_x_to_read)
        ...
        # write part
        if (quantity_of_x_to_write <= 0) or (quantity_of_x_to_write > 123) or (
                byte_count_to_write != (quantity_of_x_to_write * 2)):
            # maximum allowed size is 123 registers in one reading
            raise ModbusError(defines.ILLEGAL_DATA_VALUE)

        # look for the block corresponding to the request
        block, offset = self._get_block_and_offset(defines.HOLDING_REGISTERS, starting_write_address,
                                                   quantity_of_x_to_write)
...
class Databank(object):
    ...
    def add_slave(self, slave_id, unsigned=True, memory=None, fingerprint=None):
        ...
        with self._lock:
            ...
            if slave_id not in self._slaves:
                self._slaves[slave_id] = Slave(slave_id, unsigned, memory, fingerprint)
...
class Server(object):
    ...
    def add_slave(self, slave_id, unsigned=True, memory=None, fingerprint=None):
        ...
        return self._databank.add_slave(slave_id, unsigned, memory, fingerprint)
