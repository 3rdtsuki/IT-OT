...
def query_one_slave(slave: modbus_tcp.TcpMaster):
    ...
    logger.debug("LC:// 1")
    co_data = slave.execute(1, cst.READ_COILS, 0, 1)[0]
    logger.info("co_data: {}".format(co_data))
    slave.close()

    logger.debug("LC:// 2")
    di_data = slave.execute(2, cst.READ_DISCRETE_INPUTS, 0, 1)[0]
    logger.info("di_data: {}".format(di_data))
    slave.close()

    logger.debug("LC:// 3")
    hr_data = slave.execute(3, cst.READ_HOLDING_REGISTERS, 0, 1)[0]
